...
easyshare_sub$chronic_mod[which(easyshare_sub$chronic_mod==c("7","8","9"))]=NA 
# smoke
...
easyshare_sub$mar_stat=easyshare_sub$mar_stat %>% fct_collapse(" married" = c("1","2","3"))
levels(easyshare_sub$mar_stat) <- list("never married" = "4", "divorced" = "5", "widowed" = "6")
# sleep
...
```{r}
library(forcats)
easyshare_sub$chronic_mod=easyshare_sub$chronic_mod %>% fct_collapse("5" = c("6","7","8","9"))
```

```{r}
easyshare_sub = select(easyshare_sub, -wave)
na.omit(easyshare_sub)
```
