...
easyshare_sub$chronic_mod <- factor(easyshare_sub$chronic_mod)
...
# select variables
easyshare <- easyshare %>%  dplyr::select(c(wave, age, female, country_mod, isced1997_r, chronic_mod, ever_smoked, smoking, br010_mod, eurod, bmi2, sp002_mod, sp008_, br015_, euro5, mar_stat, recall_1, recall_2))
# rename
easyshare <- easyshare %>% rename(education = isced1997_r, drinking = br010_mod, depression = eurod, vigorous_act = br015_, sleep = euro5, recieve_help = sp002_mod, give_help = sp008_)
...
#levels(easyshare_sub$social_help) <- list("low" = "2", "moderate" = "6", "high" = "10")
...
easyshare_sub = subset(easyshare_sub, select=-c(wave, country_mod, recall_1, recall_2, mar_stat,ever_smoked, smoking))
#### drop variables NA
...
```{r}
# add the line from female to cogscore: can not add
dag=easyshare_sub.hc
dag=set.arc(dag, from = "female", to= "cogscore")
bn.cv(data = easyshare_sub_narm, bn = dag, loss = "pred", loss.args = list(target = "cogscore"))
```


```{r}
# drop the line from drinking to smoke: can drop
dag2=easyshare_sub.hc
dag2=drop.arc(dag2, from = "drinking", to= "smoke")
bn.cv(data = easyshare_sub_narm, bn = dag2, loss = "pred", loss.args = list(target = "cogscore"))
```

```{r}
# add the line from country_area to cogscore: can add
dag3=dag2
dag3=set.arc(dag3, from = "country_area", to= "cogscore")
...
```{r}
# drop the line (test)
dag4=easyshare_sub.hc
dag4=drop.arc(dag4, from = "education", to= "vigorous_act")
bn.cv(data = easyshare_sub_narm, bn = dag4, loss = "pred", loss.args = list(target = "cogscore"))
```
...
```{r}
# fit parameters
bn.bayes = bn.fit(dag2, data=easyshare_sub_narm, method = "bayes",iss=10)
bn.bayes$education
